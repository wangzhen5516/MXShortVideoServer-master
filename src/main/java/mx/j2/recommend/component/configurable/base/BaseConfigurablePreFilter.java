...
import mx.j2.recommend.component.configurable.config.PreFilterConfig;
 ...
 * @version 1.0
 * @date 2021/5/12 下午8:18
 * @description
 */
public abstract class BaseConfigurablePreFilter<T, D extends BaseDataCollection>
        extends BaseConfigurableStreamComponent<D, PreFilterConfig<T, D>>
        implements IPreFilter<D> {
    ...
    public void registerConfig(Map<String, Class> outConfMap) {
        outConfMap.put(PreFilterConfig.KEY_SKIP, ISkip.class);
        outConfMap.put(PreFilterConfig.KEY_PASS, ICheckList.class);
        outConfMap.put(PreFilterConfig.KEY_TEST, ICheck.class);
    }

    @Override
    public PreFilterConfig<T, D> newConfig(Map<String, ConfigValuePair> confMap) throws Exception {
        return new PreFilterConfig<>(confMap);
    }
