...
public class UserProfileRecallCassandraEmbeV2WithV11 extends BaseRecall<BaseDataCollection> {
    ...
    public void recall(BaseDataCollection baseDc) {
        List<String> poolDocIdList = new ArrayList<>();
        if (null != baseDc.poolToDocumentListMap.get("taka_flowpool_lv11")) {
            for (BaseDocument doc : baseDc.poolToDocumentListMap.get("taka_flowpool_lv11")) {
                poolDocIdList.add(doc.id);
            }
        }
        if (poolDocIdList.isEmpty()) {
            return;
        }
        ...
        if (MXJudgeUtils.isNotEmpty(cacheDocumentList)) {
            List<BaseDocument> mergedList = new ArrayList<>(cacheDocumentList.size());
            for (BaseDocument doc : cacheDocumentList) {
                if (poolDocIdList.contains(doc.id)) {
                    mergedList.add(doc);
                }
            }
        ...
        //根据视频id获取视频详情
        List<BaseDocument> docList = MXDataSource.details().get(scoreMap.keySet(), getName(), processor);

        //根据score排序
        docList.sort((doc0, doc1) -> Double.compare(doc1.scoreDocument.manualTopScore, doc0.scoreDocument.manualTopScore));
        localCacheDataSource.setScoreWeightRecallCache(cacheKey, docList, CACHE_TIME_SECONDS);

        List<BaseDocument> mergedList = new ArrayList<>(docList.size());
        for (BaseDocument doc : docList) {
            if (poolDocIdList.contains(doc.id)) {
                mergedList.add(doc);
            }
        }
