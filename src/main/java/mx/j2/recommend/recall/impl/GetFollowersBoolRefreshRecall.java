 ...
 */
public class GetFollowersBoolRefreshRecall extends BaseRecall<FeedDataCollection> {

    public static final String PUBLISHER_ID = "publisher_id";
    public static final String REQUEST_URL_FORMAT="/%s/_search?pretty=false";

    private static final long TEN_HOURS_MILLIS_SECONDS = 10 * 60 * 60 * 1000;

    private static final long TWO_HOURS_MILLIS_SECONDS = 2 * 60 * 60 * 1000;
    ...
    public void recall(FeedDataCollection baseDc){
        ...
        long requestTime;
        try {
            requestTime = Long.parseLong(baseDc.req.getLastRefreshTime());
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }

        boolean lessThen = requestTime + TWO_HOURS_MILLIS_SECONDS > System.currentTimeMillis();
        if (lessThen) {
            baseDc.data.response.setRedDot(false);
            baseDc.redDotInfo = "time-less-2";
            return;
        }

        boolean moreThen = requestTime + TEN_HOURS_MILLIS_SECONDS < System.currentTimeMillis();
        if (moreThen) {
            baseDc.data.response.setRedDot(true);
            baseDc.redDotInfo = "time-more-10";
            return;
        }
        ...
        if (MXJudgeUtils.isNotEmpty(followerListFromCache)) {
            if (followerListFromCache.size() <= 10) {
                baseDc.data.response.setRedDot(false);
                baseDc.redDotInfo = "followers-less-10";
                return;
            }

            if (followerListFromCache.size() >= 50) {
                baseDc.data.response.setRedDot(true);
                baseDc.redDotInfo = "followers-more-50";
                return;
            }
        }
    ...
    private int processFollowers(FeedDataCollection feedDc , List<String>followers){
        ...
        String elasticSearchRequest = String.format(REQUEST_URL_FORMAT, DefineTool.CategoryEnum.FOLLOW_RED.getIndexAndType());
    ...
    private List<String>getFollowers(FeedDataCollection baseDc){
        ...
        String result = null;
        try {
            result = source.get(url, paramsMap);
        } catch (Exception e) {
            e.printStackTrace();
        }
